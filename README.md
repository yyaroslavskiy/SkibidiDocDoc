...
**Анализ врачей Москвы: СберЗдоровье и ПроДокторов**
  ...
  * Вы можете увидеть презентацию проекта в файле `presentation.pdf`
...
**Этапы работы**
- Сбор и структурирование данных о врачах Москвы с обеих платформ.
- Сравнительный анализ стоимости приёма, рейтингов, географии распределения специалистов.
- Выявление лучших врачей и районов, где сочетание цены, рейтинга и доступности является наиболее выгодным для пациента.
...
**Парсинг Сберздоровья**
Для парсинга используется BeautifulSoup и Selenium для настройки Chrome, создания окна браузера. Использование именно Selenium обосновано динамической структурой сайта и использованием JavaScript. Опытным путём была получена информация, что в лоб парсить html с помощью реквеста не получится, на выходе получается пустой html-файл. Процесс парсинга разбит на функции согласно полям датасета. После сбора данных переменных с профиля врача все они собираются в общей функции parsedoctor. 
Сам процесс устроен так: на вход подаётся первая страница вкладки конкретной специализации врача, со страницы со спискам собираются ссылки на профили врачей, затем итерационно каждый профиль парсится. Благодаря просто структуре сайта для того чтобы перейти на следующую страницу, достаточно заменить число х в page/x. Итерационно страницы парсятся пока не дойдут до последней и не появится пустая страница без профилей врачей. Все промежуточные данные фиксируются в jsonl файле, процесс фиксируется логгированием. Затем по завершении парсинга jsonl файлы были объединены и преобразованы в csv.

Подготовка к merge:
- очищены дубликаты по ссылке (возникли из-за специалистов, которые совмещают несколько специальностей, их мы спарсили дважды)
- очищены строки с несчитанной ссылкой
- очищены строки с отсутствием значений во всех информативных признаках
